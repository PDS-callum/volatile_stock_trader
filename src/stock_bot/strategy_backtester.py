...
class StrategyBacktester:
    ...
    def __init__(self, strategy):
        # Check if strategy has required methods instead of checking type
        if not hasattr(strategy, 'check_signals') or not hasattr(strategy, 'data'):
            raise ValueError("strategy must have 'check_signals' method and 'data' attribute.")
    ...
    def run(
        ...
        if hasattr(self.strategy, '_last_trade_idx'):
            self.strategy._last_trade_idx = -1
        if hasattr(self.strategy, '_entry_price'):
            self.strategy._entry_price = None
        if hasattr(self.strategy, '_peak_price'):
            self.strategy._peak_price = None
        ...
        for i, close_price in enumerate(self.data['Close']):
            ...
            # Let the strategy handle its own position management and risk controls
            if signal == "BUY":
                buy_x.append(i)
        ...
        if plot and slider:
            try:
                ...
                def launch_dashboard():
                    ...
                    # Create dashboard data with safe column access
                    dashboard_data = [
                    ...
                    # Add EMA lines if they exist
                    if 'EMA_fast' in self.data.columns:
                        dashboard_data.append(go.Scatter(x=x_vals, y=self.data['EMA_fast'], mode='lines', name='EMA Fast', line=dict(dash='dash', color='orange')))
                    if 'EMA_slow' in self.data.columns:
                        dashboard_data.append(go.Scatter(x=x_vals, y=self.data['EMA_slow'], mode='lines', name='EMA Slow', line=dict(dash='dash', color='purple')))
                    
                    equity_curve_fig = {
                        'data': dashboard_data,
                        'layout': go.Layout(title='Equity Curve & Signals', xaxis={'title': 'Index'}, yaxis={'title': 'Price'})
                    }
                    # Create RSI figure with safe column access
                    rsi_data = []
                    if 'RSI' in self.data.columns:
                        rsi_data.append(go.Scatter(x=x_vals, y=self.data['RSI'], mode='lines', name='RSI', line=dict(color='blue')))
        ...
        # Return final equity as percentage change from initial equity
        final_equity_pct = equity_curve[-1] if equity_curve else 0.0
        return final_equity_pct

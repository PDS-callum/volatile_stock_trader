#!/usr/bin/env python3
"""
Debug the backtester with detailed output
"""
...
def main():
    ...
    # Create backtester and manually trace through the run method
    backtester = StrategyBacktester(strategy)
    
    # Manually trace through the backtester logic
    ...
    for i, close_price in enumerate(data['Close']):
        ...
        if signal == "BUY":
            ...
            print(f"Backtester BUY at index {i}, price: {close_price:.2f}")
            ...
            print(f"Backtester SELL at index {i}, price: {close_price:.2f}, trade return: {trade_return:.2f}%, cumulative: {cumulative_return:.2f}%")
            entry_price = None
    ...
    print(f'\nTesting actual backtester...')
    final_equity = backtester.run(final_equity_only=True)
